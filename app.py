...
# Try to import YOLO with graceful fallback
...
def load_model():
    if not YOLO_AVAILABLE:
        return None, "ultralytics not importable"
    try:
...
def apply_occlusion(frame, boxes, keep_n=3, style="dim", dim_alpha=0.65, blur_ksize=31):
    ...
    for i,(x1,y1,x2,y2) in enumerate(boxes):
        ...
        if style == "blur":
            ...
            overlay = roi.copy(); overlay[:] = (0,0,0)
...
def write_video(frames, fps, out_path):
    ...
    writer.release(); return out_path
...
def main():
    ...
    with open(src_path, "wb") as f: f.write(video_file.read())
    ...
    if err:
        st.warning("Running in **no-ML demo mode** (YOLO not available). You can still upload and render, but players won't be auto-detected.")
    ...
    while True:
        ...
        b = detect_people(model, rgb, conf=conf_thr)
        occluded = apply_occlusion(rgb, b, keep_n=keep_n, style=occlusion_mode)
    ...
    st.markdown("---")
    st.caption("Note: If auto-detection failed to install, the app runs in demo mode without detections.")
